...
``bamm -c control_consensus_eric.txt``
...
```{r}
is.ultrametric(mcc.trees.eric[["tree_2241"]])
is.binary.tree(mcc.trees.eric[["tree_2241"]])
# Now to check min branch length:
min(mcc.trees.eric[["tree_2241"]]$edge.length)
```

Create a control file with same parameters as the consensus tree: 

```{r}
priors <- setBAMMpriors(mcc.trees.eric[["tree_2241"]], outfile = NULL)
write.tree(mcc.trees.eric[["tree_2241"]], 'mcc_eric_2241.tre')

# Here is a block of parameters for the control file
params = list(treefile = 'mcc_eric_2241.tre',
            sampleFromPriorOnly = 0,
            runMCMC = 1,
            initializeModel = 1,
            useGlobalSamplingProbability = 1,
...
bammcontrolfile <- paste("control_", "mcc_eric_2241", ".txt", sep="")
