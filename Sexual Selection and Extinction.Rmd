...
Or perhaps OU model rather than Brownian: 
...
Try looking at speciation rate using ES-Sim adapted by @Harvey_essim_2017 and found here https://github.com/mgharvey/ES-sim
...
We can then add the essim.results and run a similar PGLS model
```{r}
restricted.data$ESsim <- essim.results[["es"]]

restricted.data %>% ggplot(aes(x=BIO12.LGM.Diff, y = (ESsim))) + 
  geom_point()+
  geom_smooth(method = "lm")
```
 
 Once we have our EsSim results we can use them in a linear model to assess the relationship between speciation and our predictors
 
```{r}
model.2 <- lm(ESsim ~ SDi 
                + range.size.m2 
                + bioclim4
                + bioclim15
                + residuals.PC1
...
Have a look at how these would look if we obtained model predictions for each data point. 

```{r, fig.width=10, fig.height=5}
predict.frame <- cbind(predict(model.2),restricted.data)
predict.frame %>% ggplot(aes(y = `predict(model.2)`, x = SDi)) + 
