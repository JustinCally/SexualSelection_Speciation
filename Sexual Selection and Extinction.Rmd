...
R-squared for relationship between RGB and Spectrophotometry Data:
```{r}
summary(lm(SDi ~ Colour.discriminability,
   data = MCC.Armenta.model.data))$r.squared %>% pander()

cor(MCC.Armenta.model.data$SDi, MCC.Armenta.model.data$Colour.discriminability)
```
...
#Run a corPagel model to estimate lambda for DR
MCC.Lambda.Armenta.global <- gls(log(mean.lambda) ~ Colour.discriminability
                         + log(range.size.m2)
                         + bioclim4 #Seasonal variation
                         + residuals.PC1 #Spatial variation
                         + PC1.LIG #Long-term climate variation
...
#Run a corPagel model to estimate lambda for DR
MCC.Extinction.Armenta.global <- gls(log(mean.mu) ~ Colour.discriminability
                         + log(range.size.m2)
                         + bioclim4 #Seasonal variation
                         + residuals.PC1 #Spatial variation
                         + PC1.LIG #Long-term climate variation
...
Armenta.data$Colour.discriminability <- abs(Armenta.data$Colour.discriminability)
...
Armenta.MCC.Lambda.top <- gls(log(mean.lambda) ~ Colour.discriminability
                      + log(range.size.m2)
                      + bioclim4 #Seasonal variation
                      + residuals.PC1 #Spatial variation
                      + PC1.LIG #Long-term climate variation
...
Armenta.MCC.Mu.top <- gls(log(mean.mu) ~ Colour.discriminability
                      + log(range.size.m2)
                      + bioclim4 #Seasonal variation
                      + residuals.PC1 #Spatial variation
                      + PC1.LIG #Long-term climate variation
...
Armenta.BAMM.lambda.pgls.models <- mcmapply(function(x,y) {
  gls(log(mean.lambda) ~ Colour.discriminability 
         + log(range.size.m2)
         + bioclim4 #Seasonal variation
         + residuals.PC1 #Spatial variation
         + PC1.LIG #Long-term climate variation
...
Armenta.BAMM.mu.pgls.models <- mcmapply(function(x,y) {
  gls(log(mean.mu) ~ Colour.discriminability
         + log(range.size.m2)
         + bioclim4 #Seasonal variation
         + residuals.PC1 #Spatial variation
         + PC1.LIG #Long-term climate variation
...
Armenta.hpd.ND.top[[x]] = hdi(Armenta.ND.pgls.summary %>% filter(Parameter == x) %>% dplyr::select(Estimate)Colour.discriminability
}
Armenta.hpd.ND.top <- bind_rows(Armenta.hpd.ND.top) %>% `rownames<-`(c("Lower", "Upper")Colour.discriminability
...
SS.subset %>% ggplot(aes(x = SDi, y = Sexual_selection_ppca))+
  ...
  theme_minimal()
```
**Figure S10:** The relationship between absolute sexual dichromatism and the sexual selection. Although positively correlated the distribution is very noisy with species having high sexual dichromatism and low sexual selection and visa versa.Colour.discriminability
```{r}
# summary(lm(SDi ~ Sexual_selection_ppcaColour.discriminability
#    data = SS.subset))$r.squared %>% pander()

cor(SS.subset$SDi, SS.subset$Sexual_selection_ppca)
```
...
BAMM.Rates.100Trees$mean.lambda <- log(BAMM.Rates.100Trees$mean.lambda)
...
BAMM.Lambda.100Trees %>% ggplot(aes(x = mean.lambda, y = DR.Lambda.100Trees$DR))+
  geom_point()+
  geom_errorbarh(aes(xmin = mean.lambda - 1.96*sd, xmax = mean.lambda + 1.96*sd), size = 0.05)+
  geom_errorbar(aes(ymin = DR.Lambda.100Trees$DR - 1.96*DR.Lambda.100Trees$sd, ymax = DR.Lambda.100Trees$DR + 1.96*DR.Lambda.100Trees$sd), size = 0.05)+
  theme_minimal()
...
```{r, eval = FALSE}
all.trees.join <- left_join(DR.Lambda.100Trees %>% select(TipLabel, DR), BAMM.Rates.100Trees %>% select(mean.lambda, TipLabel))
...
```{r, eval = FALSE}
MCC.tree.join <- left_join(restricted.data %>% select(MCC.DR, TipLabel), MCC.BAMM.model.data %>% select(mean.lambda, TipLabel))
cor(MCC.tree.join$MCC.DR, MCC.tree.join$mean.lambda)
```

```{r, eval = FALSE}
MCC.BAMM.df$CV.lambda <- (sqrt(MCC.BAMM.df$var.lambda) / MCC.BAMM.df$mean.lambda)*100
mean(MCC.BAMM.df$CV.lambda)
min(MCC.BAMM.df$CV.lambda)
max(MCC.BAMM.df$CV.lambda)

MCC.BAMM.df %>% ggplot(aes(x = mean.lambda, y = CV.lambda)) +
                         geom_point()+
  scale_y_log10()+
  scale_x_log10()
```
